...
public enum AnimationData {
    ...
    private transient final Animation<TextureRegion> animation;

    AnimationData(SpriteSheet spriteSheet, int startFrame, int endFrame, float frameDuration) {
        animation = new Animation<TextureRegion>(frameDuration, spriteSheet.getFrames(startFrame, endFrame));
    }
}
